...
hssm  <- function (d, model = "hDCRWS", adapt, samples, thin, chains, span)
{
  ssm1 <- function(dd) {
    ...
    with(dd, list(id = id, y = y, ts = ts, idx = idx, w = ws, itau2 = itau2, nu = nu, 
                  Nx = nrow(xs), Ny = nrow(dd$y), xs = xs, isigma2 = isigma2, rho = rho))
  }
  prep <- lapply(d, ssm1)
  
  ## data
  N <- length(prep) 
  Nx <- sapply(prep, function(x) x$Nx)
  Ny <- sapply(prep, function(x) x$Ny)
  y <- do.call(rbind, lapply(prep, function(x) x$y))
  itau2 <- do.call(rbind, lapply(prep, function(x) x$itau2))
  nu <- do.call(rbind, lapply(prep, function(x) x$nu))
  w <- as.numeric(unlist(lapply(prep, function(x) x$w)))
  idx <- lapply(prep, function(x) x$idx)
  m.idx <- sapply(idx, function(x) max(x))
  ...
  rho <- sapply(prep, function(x) x$rho)
  xs <- do.call(rbind,lapply(prep, function(x) x$xs))
  ...
  data <- do.call(rbind, lapply(d, function(x) x$obs))
  if(model == "hDCRWS"){
  ...
  out <- list(summary = summary, mcmc = psamples, model = model, mcmc.settings = mcmc.settings,
              timestep = d$tstep, N = N, Nx = Nx, data = data, inits = inits)
  class(out) <- "hssm"
