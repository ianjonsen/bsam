...
#' data(ellie)
#' fit <- fit_ssm(ellie, model = "DCRW", tstep = 2, adapt = 200, samples = 200, 
#'               thin = 5, span = 0.2)
#' diag_ssm(fit)
#' map_ssm(fit)
#' plot_fit(fit)
...
#' # using trivial adapt & samples values for speed
#'  fit.s <- fit_ssm(ellie, model = "DCRWS", tstep = 2, adapt = 200, samples = 200, 
#'                 thin = 5, span = 0.2)
#'  diag_ssm(fit.s)
#'  map_ssm(fit.s)
#'  plot_fit(fit.s)
...
#' # using trivial adapt & samples values for speed
#'  hfit.s <- fit_ssm(ellie, model = "hDCRWS", tstep = 2, adapt = 200, samples = 200, 
#'                 thin = 5, span = 0.2)
#'  diag_ssm(hfit.s)
#'  map_ssm(hfit.s)
#'  plot_fit(hfit.s)
